...
module.exports = function (socket) {
  ...
  socket.on('update:intensity', function (data) {
    ...
    bfgminer.updateIntensity(miner, device, value, function (err, data) {
      if (err || data.STATUS[0].STATUS == 'E') {
        socket.emit('error:intensity', data.STATUS[0]);
      }
  ...
  socket.on('update:gpuengine', function (data) {
    ...
    bfgminer.updateGpuEngine(miner, device, value, function (err, data) {
      if (err || data.STATUS[0].STATUS == 'E') {
        socket.emit('error:gpuengine', data.STATUS[0]);
      }
  ...
  socket.on('update:gpumemory', function (data) {
    ...
    bfgminer.updateMemoryClock(miner, device, value, function (err, data) {
      if (err || data.STATUS[0].STATUS == 'E') {
        socket.emit('error:gpumemory', data.STATUS[0]);
      }
  ...
  socket.on('update:gpuvoltage', function (data) {
    ...
    bfgminer.updateGpuVoltage(miner, device, value, function (err, data) {
      if (err || data.STATUS[0].STATUS == 'E') {
